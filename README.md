...
5. **文件**: 聊天视图点 "Send file" 选文件发送。入站按信任状态处理：Verified 自动接收、TOFU 弹窗确认、Untrusted（无信任记录）直接拒绝。收完按 SHA-256 校验，通过才落到系统 Downloads 目录
6. **语音**: 聊天视图点 "Voice"。被叫弹窗接受/拒绝（同意前不占麦）。Untrusted 入站邀请直接拒绝。全进程同时一路通话。通话期间同一 Session 仍能发文字
7. **通话时请使用耳机**（v1 不含回声消除；只用系统默认设备，无设备选择界面）
8. v1 使用 n0 公共 relay（hobby，无 SLA）。直连失败时音视频可能被限速
9. v1 已知限制：文件传输期间文字/信令会排在 64KiB 分块之后（同一条可靠流的队头阻塞）；传输中断开连接则该次传输失败、不续传
